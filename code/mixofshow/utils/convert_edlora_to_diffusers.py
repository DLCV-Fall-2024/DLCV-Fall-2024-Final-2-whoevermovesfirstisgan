...
def load_new_concept(pipe, new_concept_embedding, enable_edlora=True, tokenizer_idx=1, n_layers=70):
    ...
    # ==== tiwang modify start ===========================================
    # assgin the number of embeddings
    if enable_edlora:
        num_new_embedding = 16
    else:
        num_new_embedding = 1

    # fetch the codebook of the pretrained SD
    vocab = tokenizer.get_vocab()
    
    # retrieve the last token
    last_token = list(vocab.keys())[-1]

    # count how many LoRAs have been fused to SD
    # try to remove "<new" and ">" from the last token
    # the number should be divided by 16 (EDLoRA) or 1 (LoRA)
    start_idx = -1
    try:
        last_num = int(last_token.replace('<new', '').replace('>', ''))
        while start_idx * num_new_embedding < last_num:
            start_idx += 1
    # except, there is no fused LoRA
    except:
        start_idx = 0
    # ==== tiwang modify end =============================================
    ...
    for idx, (concept_name, concept_embedding) in enumerate(new_concept_embedding.items()):
        if enable_edlora:
            ...
            num_new_embedding = 1
        new_token_names = [f'<new{(idx + start_idx) * num_new_embedding + layer_id}>' for layer_id in range(num_new_embedding)]
