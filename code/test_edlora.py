...
# NEGATIVE_PROMPT += ', blue cat ear'
# NEGATIVE_PROMPT += ', purple cat ear'
# NEGATIVE_PROMPT += ', red cat ear'
# NEGATIVE_PROMPT += ', human face'
...
def test(root_path, args):
    ...
    pretrained_model_path = opt['models']['pretrained_path']

    try:
        with open(f'{pretrained_model_path}/new_concept_cfg.json', 'r') as fr:
            old_concept_cfg = json.load(fr)
        old_cfg = True
    except:
        old_cfg = False
    ...
    for lora_alpha in opt['val']['alpha_list']:
        ...
        if old_cfg:
            concept_cfg = old_concept_cfg | new_concept_cfg
        else:
            concept_cfg = new_concept_cfg
            
        pipe.set_new_concept_cfg(concept_cfg)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-opt', type=str, default='configs/edlora/test/corgi_dog.yml')
